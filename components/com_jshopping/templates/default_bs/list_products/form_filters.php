...
<?php if ($this->config->show_sort_product || $this->config->show_count_select_products){?>
    <div class="row-fluid block_sorting_count_to_page">
        ...
        <span class="span6 box_products_count_to_page"><?php echo _JSHOP_DISPLAY_NUMBER.": ".$this->product_count?></span>
    <?php }?>	
    <?php if ($this->config->show_sort_product){?>
        <span class="span6 box_products_sorting"><?php echo _JSHOP_ORDER_BY.": ".$this->sorting?><img src="<?php echo $this->path_image_sorting_dir?>" alt="orderby" onclick="submitListProductFilterSortDirection()" /></span>
    <?php }?>
    </div>
<?php }?>
