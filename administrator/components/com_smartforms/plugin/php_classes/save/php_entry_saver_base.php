...
class php_entry_saver_base {
    ...
    {
        include_once(SMART_FORMS_PUBLIC_DIR.'smart-forms-ajax.php');
        $this->ajax=new SmartFormsAjax();
        $this->FormId = $formId;
    ...
    public function ProcessEntry($validateCaptcha=true,$additionalData=array())
    {
		...
		{
			return '{"message":"'."An error occurred, please try again later.".'","refreshCaptcha":"n","success":"n"}';
		}

        if($this->FormOptions["UsesCaptcha"]=="y"&&$validateCaptcha==true)
        {
			if($this->FormOptions["CaptchaVersion"]=="1")
				if(!$this->CaptchaIsValid2())
				{
					return  '{"message":"'.__("Invalid captcha.").'","refreshCaptcha":"y","success":"n"}';
			...
			if($this->FormOptions["CaptchaVersion"]=="2")
				...
				{
					return '{"message":"'.$errorMessage.'","refreshCaptcha":"y","success":"n"}';
		...
		SmartFormsActionManager::GetInstance()->DoAction('sf_before_saving_form',$preInsertedEntry);



		if(SmartFormsActionManager::GetInstance()->HasAction('sf_before_saving_form'))
		{
        ...
        if($result==true)
		{
			...
			SmartFormsActionManager::GetInstance()->DoAction('sf_after_saving_form',$postInsertEntry);
		...
		{
            return '{"message":"'."An error occurred, please try again later.".'","success":"n"}';
		}
    ...
    private function InsertEntryData(){
        ...
        $result=SmartFormsDBHelper::Insert('#__smartforms_entry',
            array(
                'form_id'=>$this->FormId,
                'user_id'=>$user->id,
    ...
    public function SendFormEmail($formOptions,$entryData,$elementOptions,$preInsertEntry,$useTestData)
    {
        ...
        }catch (Exception $e)
        {
            ...
            {
                $value=$this->ajax-> GetValueByField($this->StringBuilder,$match,$entryData,$elementOptions,$useTestData);
		...
		if(strpos($FromEmail,"[field")===0)
		{
			...
			{
				$field=$matches[1][0];
				$value=$this->ajax->GetValueByField($this->StringBuilder,$field,$entryData,$elementOptions,$useTestData);
				$FromEmail=$value;
			}else{
				$FromEmail="";
        ...
        if(trim($ToEmail)!="")
        {
            ...
            $emailParams=SmartFormsFilterManager::GetInstance()->ApplyFilter('smart_forms_before_sending_email',$emailParams);
			$emailContent=$emailParams["EmailContent"];
            $mail = JFactory::getMailer();
            $mail->setSender(array($FromEmail,$FromName));
            $mail->setSubject($emailContent["Subject"]);
            $mail->setBody($emailContent["EmailText"]);
            $mail->IsHTML(true);
            $mail->addRecipient($emailContent["ToEmail"]);
            ...
            $result= $mail->send();
            SmartFormsActionManager::GetInstance()->DoAction('smart_forms_after_sending_email');
	...
	private function CaptchaIsValid2(&$errorMessage)
	{
		...
		$captcha=ARRAY();
		$captcha["response"]=$captchaPost["response"];
		$captcha["remoteip"]=$_SERVER['REMOTE_ADDR'];
		$captcha["secret"]=$this->FormOptions['FieldServerOptions']['captcha2']['SecretKey'];


        $curlConfig = array( CURLOPT_URL => 'https://www.google.com/recaptcha/api/siteverify', CURLOPT_POST => true, CURLOPT_RETURNTRANSFER => true, CURLOPT_POSTFIELDS => $captcha ,CURLOPT_SSL_VERIFYPEER=>false);
        $ch = curl_init();
    ...
    {
        $db=JFactory::getDBO();
        $db->setQuery("select form_options,element_options from #__smartforms where form_id=".$db->quote($this->FormId));
	...
	private function InsertDetailRecord($entry_id, $fieldId, $value, $jsonValue,$exValue,$dateValue)
	{
        ...
        $result=SmartFormsDBHelper::Insert('#__smart_forms_entry_detail',
            $arrayToInsert);
